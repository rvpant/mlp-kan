...
def main():
    ...
    if mode == 'deep':
        if modeltype=='efficient_kan':
            ...
            branch_net1 = KANBranchNet(input_neurons_branch, [2*input_neurons_branch+1]*3, p, modeltype='cheby_kan', degree=8, layernorm=False)
            branch_net2 = KANBranchNet(input_neurons_branch, [2*input_neurons_branch+1]*3, p, modeltype='cheby_kan', degree=8, layernorm=False)
            branch_net1.to(device)
            branch_net2.to(device)
            trunk_net = KANTrunkNet(input_neurons_trunk, [2*input_neurons_trunk+1]*3, p, modeltype='cheby_kan', degree=8, layernorm=False)
            trunk_net.to(device)
            ...
            branch_net1 = KANBranchNet(input_neurons_branch, [2*input_neurons_branch+1]*3, p, modeltype='legendre_kan', degree=8, layernorm=False)
            branch_net2 = KANBranchNet(input_neurons_branch, [2*input_neurons_branch+1]*3, p, modeltype='legendre_kan', degree=8, layernorm=False)
            branch_net1.to(device)
            branch_net2.to(device)
            trunk_net = KANTrunkNet(input_neurons_trunk, [2*input_neurons_trunk+1]*3, p, modeltype='legendre_kan', degree=8, layernorm=False)
            trunk_net.to(device)
        else:
